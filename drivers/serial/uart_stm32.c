...
static inline int is_dma_rx_enabled(const struct device *dev)
{
	USART_TypeDef *UartInstance = UART_STRUCT(dev);
...
static int uart_stm32_dma_rx_enable(const struct device *dev, bool enable)
{
	USART_TypeDef *UartInstance = UART_STRUCT(dev);
	struct uart_stm32_data *data = DEV_DATA(dev);
	bool isEnabled;
...
static void uart_stm32_dma_tx_cb(const struct device *dma_dev,
